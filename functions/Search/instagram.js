...
export const meta = {
  ...
  commands: ['instagramsearch', 'igsearch', 'igsh'],
  priority: 5,
  premium: true,
  class: 'Buscadores',
}
...
export default async function (msg, sock, ctx) {
  ...
  try {
    const res = await global.OptiShield.callApi('instagram-search', { apikey, query, limit: 10 })
    ...
    menuText += `_Selecciona para descargar o ver en Instagram_`

    const rows = results.map((r, i) => {
      return {
        id: `.igdl ${r.linkReal}`,
        title: `⬇️ Resultado ${i + 1}`,
      ...
      interactiveButtons: [
        {
          ...
          buttonParamsJson: JSON.stringify({
            title: '⬇️ Seleccionar post',
            sections: [{ title: '📸 Resultados de Instagram', rows }]
          })
        },
        {
          name: 'cta_url',
