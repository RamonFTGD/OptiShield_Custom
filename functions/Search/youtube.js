...
export default async function (msg, sock, ctx) {
  ...
  const edit = async (text) => {
    try { await sock.sendMessage(chatId, { text, edit: statusMsg.key }) } catch { }
  }
  ...
  try {
    ...
    for (let i = 0; i < results.length; i++) {
      const r = results[i]
      const titulo = r.title.length > 60
      ...
      sections.push({
        ...
        rows: [
          {
            id: `.play ${r.url}`,
            title: `🎵 ${titulo.substring(0, 20)}${titulo.length > 20 ? '...' : ''}`,
            description: `MP3 • ${r.duration} • ${r.channel}`
          },
          {
            id: `.play2 ${r.url}`,
            title: `🎬 ${titulo.substring(0, 20)}${titulo.length > 20 ? '...' : ''}`,
            description: `MP4 • ${r.duration} • ${r.channel}`
          }
        ]
      })
    }
    ...
    for (let i = 0; i < results.length; i++) {
      const r = results[i]
      const titulo = r.title.length > 50
        ? r.title.substring(0, 50) + '...'
        : r.title
      menuText += `*${i + 1}.* ${titulo}\n`
      menuText += `   👤 ${r.channel} • ⏱️ ${r.duration}\n\n`
    }

    menuText += `_Selecciona 🎵 para MP3 o 🎬 para MP4_`

    await edit('✅ Resultados listos, abriendo lista...')

    await sendInteractiveWithImage(sock, chatId, {
      imageUrl: firstVideoThumb,
      bodyText: menuText,
      footerText: `OptiShield • ${results.length} videos encontrados`,
      buttons: [
        {
          name: 'single_select',
