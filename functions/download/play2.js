...
function parseDuration(durationStr) {
  try { const parts = durationStr.split(':').map(Number); if (parts.length === 2) return parts[0] * 60 + parts[1]; if (parts.length === 3) return parts[0] * 3600 + parts[1] * 60 + parts[2]; return 0 } catch { return 0 }
}
function progressBar(step, total = 6) { const filled = Math.round((step / total) * 10); const empty = 10 - filled; return `[${'█'.repeat(filled)}${'░'.repeat(empty)}] ${Math.round((step / total) * 100)}%` }
async function updateLog(sock, chatId, logKey, text) { try { if (logKey) await sock.sendMessage(chatId, { text, edit: logKey }) } catch {} }
const VIDEO_LIMIT = 1.5 * 1024 * 1024 * 1024
...
async function reloadTor() {
  const tryControlPort = () => new Promise((resolve) => {
    ...
    let authed = false
    s.on('data', (d) => { const r = d.toString().trim(); if (!authed) { if (r.startsWith('250')) { authed = true; s.write('SIGNAL NEWNYM\r\n') } else { s.destroy(); resolve(false) } } else { s.destroy(); resolve(r.startsWith('250')) } })
    s.on('error', () => resolve(false)); s.setTimeout(3000, () => { s.destroy(); resolve(false) })
  })
  ...
  if (await tryControlPort()) return true; return tryExec()
}

function httpRequest(url, options = {}) { return new Promise((resolve, reject) => { const client = url.startsWith('https') ? https : http; const req = client.get(url, options, resolve); req.on('error', reject); req.on('timeout', () => req.destroy(new Error('CONN_TIMEOUT'))) }) }
async function followRedirects(url, options = {}, maxHops = 10) {
  let currentUrl = url
  for (let i = 0; i < maxHops; i++) {
    const res = await httpRequest(currentUrl, options)
    if (res.statusCode >= 300 && res.statusCode < 400 && res.headers.location) {
      res.resume(); let nextUrl = res.headers.location
      if (nextUrl.startsWith('/')) { const parsed = new URL(currentUrl); nextUrl = `${parsed.origin}${nextUrl}` } else if (!nextUrl.startsWith('http')) { const parsed = new URL(currentUrl); nextUrl = `${parsed.origin}/${nextUrl}` }
...
async function fetchBuffer(url, maxBytes) {
  ...
  async function attempt(agent, label, timeout) {
    const options = { timeout, headers: { ...BROWSER_HEADERS } }; if (agent) options.agent = agent
    try { const res = await followRedirects(url, options); return await drainStream(res, maxBytes) } catch (err) { allErrors.push(`[${label}] ${err.message || String(err)}`); return null }
  }
  console.log(`📥 [play2] Directo: ${url.substring(0, 60)}...`)
  const directResult = await attempt(null, 'Directo', 60000)
  if (directResult) { console.log(`✅ [play2] Directo OK`); return directResult }
  await sleep(2000)
  for (const proxyUrl of SOCKS_PROXIES) {
    for (let attemptNum = 0; attemptNum < 2; attemptNum++) {
      ...
      if (attemptNum === 1) { console.log(`🔄 [play2] Reload Tor...`); await reloadTor(); await sleep(5000) }
      console.log(`🔄 [play2] Proxy ${proxyUrl} (intento ${attemptNum + 1})`)
      const proxyResult = await attempt(agent, `Proxy ${attemptNum + 1}`, 120000)
      try { agent.destroy() } catch {}
      if (proxyResult) { console.log(`✅ [play2] Proxy OK`); return proxyResult }
...
// --- FUNCIONES NATIVAS DE BAILEYS (Para la tarjeta interactiva del video) ---
function getBaileysFns(sock) {
  ...
  for (const pkg of candidates) {
    try {
      ...
      if (generateWAMessageFromContent && prepareWAMessageMedia && sock.relayMessage) return { generateWAMessageFromContent, prepareWAMessageMedia, generateMessageIDV2, isJidGroup }
    } catch (_) {}
  }
  return null
}
...
async function sendInteractiveWithImage(sock, jid, { imageUrl, bodyText, footerText, buttons, quotedMsg }) {
  ...
  if (imageUrl) { try { mediaContent = await prepareWAMessageMedia({ image: { url: imageUrl } }, { upload: sock.waUploadToServer }) } catch (err) { console.warn('⚠ No se pudo preparar imagen para el header:', err.message) } }
  ...
  additionalNodes.push({ tag: 'biz', attrs: {}, content: [{ tag: 'interactive', attrs: { type: 'native_flow', v: '1' }, content: [{ tag: 'native_flow', attrs: { v: '9', name: 'mixed' } }] }] })
...
export default async function (msg, sock, ctx) {
  ...
  try {
    ...
    const dl = await global.OptiShield.callApi('youtubedl', { url: videoUrl, video: 1, apikey })
    ...
    const duration = data.duration || parseDuration(data.durationFormatted || '0:0')

    await updateLog(sock, chatId, logKey, buildProgress(videoTitle, videoChannel, data.durationFormatted, 4))

    const safeTitle = videoTitle.normalize('NFD').replace(/[\u0300-\u036f]/g, '').replace(/[\\/:*?"<>|]/g, '').replace(/[^\x20-\x7E]/g, '').replace(/\s+/g, ' ').trim().substring(0, 100) || 'OptiShield Video'
    const thumbBuffer = await fetchThumbnail(data.thumbnail)
    const downloadUrl = data.available?.video?.[0]?.download_url

    if (!downloadUrl) { await updateLog(sock, chatId, logKey, '❌ Sin video disponible'); return true }

    await updateLog(sock, chatId, logKey, buildProgress(videoTitle, videoChannel, data.durationFormatted, 5) + '\n\n⏳ Descargando video...')
    const videoBuffer = await fetchBuffer(downloadUrl, VIDEO_LIMIT)

    // Subir thumbnail a servidor para la tarjeta interactiva
    let thumbUrlForInteractive = null
    if (thumbBuffer) {
      try { const uploadRes = await global.OptiShield.uploadFile(thumbBuffer); thumbUrlForInteractive = uploadRes.archivo } catch (e) { console.warn('⚠ Error subiendo thumbnail:', e.message) }
    }

    // Enviar tarjeta interactiva
    await sendInteractiveWithImage(sock, chatId, {
      imageUrl: thumbUrlForInteractive,
      bodyText: `🎬 *${videoTitle}*\n${videoChannel ? `👤 ${videoChannel}\n` : ''}⏱️ ${data.durationFormatted}\n\n_⬇️ Enviando video a continuación..._`,
      footerText: 'OptiShield • Descargador',
      buttons: [{ name: 'cta_url', buttonParamsJson: JSON.stringify({ display_text: '🔗 Ver en YouTube', url: videoUrl, merchant_url: videoUrl }) }],
      quotedMsg: msg
    })

    // Enviar video
    const caption = `🎬 *${videoTitle}*`
    const messageOpts = { video: videoBuffer, mimetype: 'video/mp4', fileName: `${safeTitle}.mp4`, caption }

    if (duration > 1500) {
      await sock.sendMessage(chatId, { document: videoBuffer, ...messageOpts }, { quoted: msg })
    } else {
      await sock.sendMessage(chatId, messageOpts, { quoted: msg })
    }
