...
async function sticker5(
    ...
    const buffer2 = img ? img : false;
    const { Sticker } = await import("wa-sticker-formatter");
    const buffer = await new Sticker(buffer2 ? buffer2 : url).setQuality(85).toBuffer(); //Kurt18: El valor por default es 100 (se queda en 10, porque si subo a 20 se pierde la animacion).toBuffer();
    return buffer;
  }
...
async function sticker(img, url, ...args) {
    let lastError;
    let stiker;
    for (const func of [ sticker5 ].filter((f) => f)) {
        try {
